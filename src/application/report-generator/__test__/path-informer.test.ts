...
function createFSTree(impl = () => "") {
	return {
		getShortPathByPath: jest.fn(impl),
	};
}

describe("path-informer", () => {
	it("should get root path correctly", () => {
		const pathInformer = new PathInformer({ rootPath, fSTree: createFSTree() });
	...
	it("should get module html page path correctly", () => {
		const pathInformer = new PathInformer({ rootPath, fSTree: createFSTree(() => "src/lib/lib.ts") });
	...
	it("should get package html page path correctly", () => {
		const pathInformer = new PathInformer({ rootPath, fSTree: createFSTree(() => "src/lib") });
